...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('\n        <div class="search__result">No results found for "' + _this.value + '"</div>\n      ');
    }).catch(function (err) {
      console.error(err);
    });
  });
  ...
  searchInput.on('keyup', function (e) {
    ...
    var next = void 0; // figure our which is next 
    if (e.keyCode === 40 && current) {
      // key is downarrow and activeClass is present
      next = current.nextElementSibling || items[0];
    } else if (e.keyCode === 40) {
      // key is downarrow, give first item activeClass 
      next = items[0];
      ...
      // key is enter; go to url and stop function
